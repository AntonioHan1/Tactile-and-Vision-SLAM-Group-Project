...
import time
'''
File for plotting visual loop closure (although not included in report)
'''
...
load_saved = False
save_path = '../checkpoints_12_10/vis_lc/'
...
if load_saved:
  ...
  model = VisualLoopClosureModel()
  model.load_weights(save_path + '/epoch80/vis_lc_checkpoint')
  ...
  np.save(save_path + 'vis_image_times_first30.npy', gs_image_times)
  np.save(save_path + 'vis_pred_loop_closure_steps_first30.npy', pred_loop_closure_steps)
  np.save(save_path + 'vis_actual_loop_closure_steps_first30.npy', actual_loop_closure_steps)
...
plt.savefig('../checkpoints_12_10/tactile_lc.png')