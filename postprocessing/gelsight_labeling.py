...
class LabelImages(object):
  ...
  def label(self):
    for experiment_reldir in os.listdir(self.calib_data_dir):
      ...
      frame_path = os.path.join(experiment_dir, "image_crop.png")
      ...
      while True:
        ...
        if key == ord('y') or key == ord('n'):
          if key == ord('n'):
            ...
            x = self.buttondown_xy[1]
            y = self.buttondown_xy[0]
            print(x, y)
            np.save(os.path.join(experiment_dir, "point_in_gelsight.npy"), (x, y))
